#!/bin/bash
# claudetop-stop.sh — Sets iTerm2 background to green when Claude stops
...
# Write directly to TTY for instant green
if [ -f "$TTY_MAP" ]; then
...
if grep -q "^bgcolor=" "$STATE_FILE" 2>/dev/null; then
  sed -i "" "s/^bgcolor=.*/bgcolor=152b17/" "$STATE_FILE"
else
  echo "bgcolor=152b17" >> "$STATE_FILE"
fi
